...
class Player1InputSystem : public EventSystem {
  void run(SDL_Event event) {
    scene->r.view<SpeedComponent, Player1Component>()
      .each(
...
class Player2InputSystem : public EventSystem {
    void run(SDL_Event event) {
        scene->r.view<SpeedComponent, Player2Component>()
            .each(
                [&](const auto entity, auto& s, auto& p) {
                    if (event.type == SDL_KEYDOWN) {
                        switch (event.key.keysym.sym) {
